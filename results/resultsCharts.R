...
#~ 	print(scene)
	...
	for (alg in algs) {
		...
		passPct <- c(passPct, 100*nrow(passSet)/100)
		if (nrow(passSet) > 0) {
			...
			invScore <- 1/passSet$total_score
...
#~ 	time <- (1.5*max(time))-time
	...
	if (first== T) {
		par(fig=c(cpad,cWidth+cpad,rpad,cWidth/2+rpad), mar=cMar)
		first <- F
	}else{
		par(fig=c(cpad,cWidth+cpad,rpad,cWidth/2+rpad), mar=cMar, new=T)
	}
	
	colors <- c("deepskyblue", "darkgreen", "burlywood3", "cyan4", "darkorange3", "darkolivegreen1", "purple")
	
	base_r_barplot <- barplot(qual, main=scene, sub="Quality", ylim = c(0, max(qual)*0.1 + max(qual)), col=colors)
	...
	barplot(passPct, sub="Availability", col=colors, ylim=c(0,100))
	par(fig=c(0.005+cWidth/2+cpad,cWidth+cpad,cWidth/2+rpad,cWidth+rpad), new=TRUE, mar=cMar)
	time_plot <- barplot(time, sub="Performance", col=colors)
