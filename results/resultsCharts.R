...
#~ 	print(scene)
	...
	for (alg in algs) {
		...
		passPct <- c(passPct, nrow(passSet)/100)
		if (nrow(passSet) > 0) {
...
#~ 	total <- passPct*((qual*7 + time*3)/10)
	total <- passPct*qual
	
	allAgg <- allAgg + total

	cMar <- c(5,0.5,0.5,0.5)
	...
	base_r_barplot <- barplot(qual, main=scene, sub="Quality", ylim = c(0, 1.0), col=colors)
	...
	barplot(total, main=scene, sub="Aggregate", ylim = c(0, 1), col=colors)

	par(fig=c(cpad,cWidth/2+cpad,cWidth/2+rpad,cWidth+rpad), new=TRUE, mar=cMar)
	barplot(passPct, sub="Availability", col=colors, ylim=c(0,1))
	
	par(fig=c(0.005+cWidth/2+cpad,cWidth+cpad,cWidth/2+rpad,cWidth+rpad), new=TRUE, mar=cMar)
	time_plot <- barplot(time, sub="Performance", col=colors, ylim=c(0,1))
...
barplot(allAgg/9, main=scene, sub="Aggregate", ylim = c(0, 1.0), col=colors)
dev.off()
